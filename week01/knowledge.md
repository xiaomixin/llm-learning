...
### 2.3.1 图解：同一分类器，同一阈值，两种指标讲两个故事

下图是一个**完全合成的、可复现的**实验，规模和分布都对标 Kaggle creditcardfraud（N=10000，正样本 17 个 ≈ 0.17%）：正类得分 $\sim \mathcal{N}(2, 1)$，负类 $\sim \mathcal{N}(0, 1)$。分类器足够好（类别均值差 2σ），但我们在两条曲线上都标出**同一个阈值点** `thr=2.31`（对应 FPR≈0.01，这是欺诈场景最常看的工作点）。

![ROC vs PR on imbalanced data](assets/roc_vs_pr.png)

**这张图要你盯住的三件事：**

1. **同一个分类器的 AUC 差了 7 倍**：AUC-ROC = **0.957**（看上去"接近满分"），AUC-PR = **0.140**（看上去"惨不忍睹"）。不是两个模型的对比——是**同一个模型的两种报告方式**。

2. **同一阈值 `thr=2.31` 的混淆矩阵是什么？**

   |                 | 预测正 | 预测负 |
   |-----------------|-------|-------|
   | 实际正（17）    | **TP=9**  | FN=8   |
   | 实际负（9983）  | **FP=112** | TN=9871 |

   在 ROC 曲线上你看到的是：`FPR = 112 / 9983 = 1.12%`，分母里有 **9983 个真负样本**稀释着，红点看起来很靠近左上角（好！）。
   在 PR 曲线上你看到的是：`Precision = 9 / (9 + 112) = 7.4%`，分母里**没有那 9871 个真负样本**，直接暴露了"报警 121 次里只有 9 次是真的"的业务灾难。

3. **PR 曲线的下界不是 0.5 而是 $\pi$**：左图的 Random baseline 是 y=x（AUC=0.5），右图的 Random baseline 是水平线 $y=\pi=0.0017$（AP≈0）。偏斜越严重，PR 的"零点"越低，PR 数字的变化范围也越极端——**同一个 PR=0.30 在平衡数据上很差，在 0.17% 数据上可能是 SOTA**。这也是为什么 PR 数字**不能跨数据集比较**。

**复现方法**：`week01/assets/generate_roc_pr_chart.py`，30 秒跑完。你可以改 `N_POS` / `scores_pos` 的 μ 值，观察两条曲线如何随正样本比例和分类器强度变化——把这当成本周的"指标直觉练习"，比读 10 遍 Davis & Goadrich 的推导都管用。

**口诀（写下来贴屏幕上）**：
- **ROC 分母是"负样本池"，池子很大 → 画面很漂亮。**
- **PR 分母是"报警池"，报警的大多数是假的 → 画面很扎心。**
- **业务关心"每次报警是不是真欺诈"，所以看 PR。**
